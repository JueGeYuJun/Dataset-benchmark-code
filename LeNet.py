...
# 配置 logging
log_filename = 'logs/LeNet/LeNetTest3'  # 日志文件的路径
...
augmentation_transform = transforms.Compose([
    ...
    transforms.RandomRotation(degrees=15),  # 图像随机旋转
    transforms.RandomResizedCrop(size=(128, 128)),  # 随机截取并resize
transforms.RandomAffine(degrees=0, translate=(0.5, 0.5), scale=(0.25, 1)),  # 随机仿射变换
])
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(  # 输入的图片 （1，28，28）
                in_channels=1,
                out_channels=16,  # 经过一个卷积层之后 （16, 64, 64）
                kernel_size=5,
        ...
        self.conv2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2)  # 输出（32, 16, 16）
        )

        self.out = nn.Linear(32 * 16 * 16, num_classes)

    def forward(self, x):
        x = self.conv1(x)  # （batch_size,16,32,32）
        x = self.conv2(x)  # 输出（batch_size,32,16,16）
        x = x.view(x.size(0), -1)  # (batch_size,32*16*16)
